...
class FileFlowFlutterApi(private val binaryMessenger: BinaryMessenger, private val messageChannelSuffix: String = "") {
  ...
  fun updateTaskProgress(taskIdArg: String, progressArg: Long, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.file_flow.FileFlowFlutterApi.updateTaskProgress$separatedMessageChannelSuffix"
  ...
  fun updateTaskState(taskIdArg: String, stateArg: TaskState, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.file_flow.FileFlowFlutterApi.updateTaskState$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(taskIdArg, stateArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(FlowFlutterError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
